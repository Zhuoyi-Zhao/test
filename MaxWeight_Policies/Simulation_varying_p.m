...
for count=1:num_setups
    ...
    alpha=[4;6;5];
    arrival=[1;1;1];
    probability=vector_p(count)*[0.9;0.6;0.5];
    ...
    optimal_cost_Rand(count)=(sum(sqrt(A./p.*(3*L.^2-L)./(2.*L))))^2/M+sum(A./2)/M;
    % No Switching Randomized policy
    mu_opt_no_switch = optimize_mu_for_random_policy(M, p, L, alpha, E_Y2);
    optimal_cost_Rand_NoSwitch(count) = weighted_AoI(mu_opt_no_switch, p, L, E_Y2, alpha)
    ...
    optimal_cost_Rand_NoSwitch_Simulated(count) - optimal_cost_Rand_NoSwitch(count)
...
figure(4)
hold on
[Rand_NoSwitch_plot] = plot(vector_p,optimal_cost_Rand_NoSwitch,'cs--','LineWidth',3,'MarkerSize',15);
[Rand_NoSwitch_Simulated_plot] = plot(vector_p,optimal_cost_Rand_NoSwitch_Simulated,'ys--','LineWidth',3,'MarkerSize',15);
[Rand_plot]=plot(vector_p,optimal_cost_Rand,'bo--','LineWidth',3,'MarkerSize',20);
...
legend([Rand_plot, Rand_NoSwitch_Simulated_plot,Linear_Max_Weight_plot_Age_debt, Linear_Max_Weight_plot_Greedy, Linear_Max_Weight_plot_QuaDPP, Lower_Bound, Rand_NoSwitch_plot],...
    'Random (Switch)','Random (No-Switch)simulated','Age Debt','Greedy','LinearMW','Lower Bound','Random (No-Switch)','Location','NorthWest');ylabel('Expected Weighted Sum AoI')
% legend([Rand_plot, Rand_NoSwitch_Simulated_plot,Rand_NoSwitch_plot,Lower_Bound],...
    % 'Random (Switch)','Random (No-Switch)simulated','Random (No-Switch)','Lower Bound','Location','NorthWest');ylabel('Expected Weighted Sum AoI')
