...
throughput_target = zeros(M,1);
throughput_target =  [throughput_target+ probability.*optMU ;0];
...
for iteration=1:1:num_iterations
    ...
    for slot_index=1:1:K       
        ...
        throughput_debt = max(slot_index.*throughput_target- delivered,0);
        ...
        elseif set_Policy==0 %The Greedy on the h is used
            ...
            [tilde1,node_index]=max(sqrt(A(1:M).*p(1:M)).*(max(Q+h+1-Achievable_AoI,0).^2-max(Q+z+vector_successful_tx-Achievable_AoI,0).^2).*double(vector_successful_tx>0));
...
%% QuaMW
            ...
            [tilde1,node_index]=max((sqrt(A(1:M).*p(1:M)).*(1).*double(vector_successful_tx==L) ...
                +sqrt(A(1:M).*p(1:M)).*((h-z)-(z+1)).*double(vector_successful_tx==1)...
                +2.*sqrt(A(1:M).*p(1:M)).*(z+vector_successful_tx).*double(vector_successful_tx<L) ...
                +2.*sqrt(A(1:M).*p(1:M)).*((z+2)-L).*double(vector_successful_tx==1) ...
                +throughput_debt(1:M)).*double(vector_successful_tx>0));
...
%% W+S+WS
        ...
        if (node_index~=(M+1)) && (rand<p(node_index)) %&& (vector_successful_tx(node_index)>=1)
            ...
            % Q(node_index) = max(Q(node_index)+z(node_index)+L(node_index)/p(node_index)-Achievable_AoI(node_index),0);
            % Q(node_index) = max(Q(node_index)+z(node_index)+L(node_index)-Achievable_AoI(node_index),0);
            % Q(node_index) = Q(node_index)+z(node_index)+L(node_index)-Achievable_AoI(node_index);
            % Q(node_index) = max(Q(node_index)+z(node_index)/L(node_index)+1-Achievable_AoI(node_index),0);
            % Q(node_index) = max(Q(node_index)+z(node_index)/L(node_index)+1/p(node_index)-Achievable_AoI(node_index),0);
            % Q(node_index) = max(Q(node_index)+z(node_index)+vector_successful_tx(node_index)+1-Achievable_AoI(node_index),0);
            % Q(node_index) = max(Q(node_index)+(z(node_index)+vector_successful_tx(node_index)/p(node_index)./optMU(node_index)+1).^2-Achievable_AoI(node_index).^2,0);
            Q(node_index) = max(Q(node_index)+(z(node_index)+vector_successful_tx(node_index)+1)-Achievable_AoI(node_index),0);
            % Q(node_index) = Q(node_index)+z(node_index)/L(node_index)+1/p(node_index)-Achievable_AoI(node_index);
