...
simulationLength = 1000; % Number of time steps per simulation

packetSwitchingCount = 0; % Record the number of times packet switching occurs

for sim = 1:numSimulations
    [switchOccurred] = simulateSystem(N, L_i, p_i, A_max, policy, stateList, stateMap, initialState, simulationLength);
    if switchOccurred
...
function [switchOccurred] = simulateSystem(N, L_i, p_i, A_max, policy, stateList, stateMap, initialState, simulationLength)
    ...
    for t = 1:simulationLength
        ...
        elseif action ~= ongoingSource
            ...
            switchOccurred = true;
            break; % Decide whether to exit immediately or continue simulation as needed
        end
